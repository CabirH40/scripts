...
def check_log_for_completed():
    ...
    try:
        ...
        if "authentication complete" in content:
            ...
            send_message_to_server(f"تمت عملية التوثيق بنجاح نراك بعد أسبوع", phone) # تم تعديل الاستدعاء هنا
            alert_sent = True
...
# وظيفة لإفراغ الملف يومياً في الساعة 3 صباحاً
def empty_log_file():
    try:
        with open(log_file_path, 'w') as log_file:
            log_file.write("")
        print("تم إفراغ الملف بنجاح.")
    except Exception as e:
        print(f"حدث خطأ أثناء إفراغ الملف: {e}")
