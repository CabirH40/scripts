...
telegram_token='7375997728:AAG7HwYA72_n25VMoCaPmt4xWJP_3D9dliA'
telegram_group='-4270399214'
telegram_user_tag="@Cabirbtc"
...
if ! pgrep -x "$process_name" > /dev/null; then
  message="🚨Server ibrahim (${server_ip}) process ${process_name} has been stopped ${telegram_user_tag}"
else
  ...
  if [ "$(echo "$status" | tr '[:upper:]' '[:lower:]')" == "$(echo '"inactive"' | tr '[:upper:]' '[:lower:]')" ]; then
    message="🚨ibrahim Humanode (${server_ip}) is not active, please proceed to do re-authentication ${telegram_user_tag}"
  else
    ...
    if (( difference > 25 && difference < 31 )); then
      message="🟡ibrahim Humanode (${server_ip}) will be deactivated in 30 minutes, please prepare for re-authentication ${telegram_user_tag}"
    elif (( difference > 0 && difference < 6 )); then
      message="🔴ibrahim Humanode (${server_ip}) will be deactivated in 5 minutes, please prepare for re-authentication ${telegram_user_tag}"
    else
      message="NULL"
    fi
  fi
