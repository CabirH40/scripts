...
if ! pgrep -x "$process_name" > /dev/null; then
  message="🚨Server cabir (${server_ip}) process ${process_name} has been stopped ${telegram_user_tag}"
else
  ...
  if [ "$(echo "$status" | tr '[:upper:]' '[:lower:]')" == "$(echo '"inactive"' | tr '[:upper:]' '[:lower:]')" ]; then
    message="🚨cabir Humanode (${server_ip}) is not active, please proceed to do re-authentication ${telegram_user_tag}"
  else
    ...
    if (( difference > 25 && difference < 31 )); then
      message="🟡cabir Humanode (${server_ip}) will be deactivated in 30 minutes, please prepare for re-authentication ${telegram_user_tag}"
    elif (( difference > 0 && difference < 6 )); then
      message="🔴cabir Humanode (${server_ip}) will be deactivated in 5 minutes, please prepare for re-authentication ${telegram_user_tag}"
    else
      message="NULL"
    fi
  fi
